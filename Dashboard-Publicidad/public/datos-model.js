...
function configurarModalEdicion() {
    ...
    document.getElementById('editarModeloModal').addEventListener('show.bs.modal', function () {
    ...
    document.querySelector('.modal-footer .btn-primary').addEventListener('click', async function () {
        ...
        if (!updatedData.cedula || !updatedData.correo || !updatedData.numero_tlf || !updatedData.edad) {
            alert('Todos los campos son obligatorios');
            return;
        }

        try {
            const response = await fetch(`http://10.100.39.23:8000/modelos/api/head/${modeloId}/`, {
                method: 'PUT',
                headers: {
