document.addEventListener('DOMContentLoaded', function() {
  ...
  async function actualizarTarjetas() {
    ...
    try {
      ...
      const pendientesElement = document.querySelector(
        "#PautasPendientes .card-value"
      );
      const activasElement = document.querySelector(
        "#PautasActivas .card-value"
      );
      const inversionElement = document.querySelector(
        "#InversionTotal .card-value"
      );
      const procesadaElement = document.querySelector(
        "#PautasProcesadas .card-value"
      );
  ...
  async function cargarTablaPautas() {
    ...
    try {
      ...
      data.lista_pautas.forEach((pauta) => {
        ...
        const authComercialText =
          pauta.autorizacio_comercial === true ||
          pauta.autorizacio_comercial === "true"
            ? "Aprobada"
            : "Rechazada";

        const authDirectivaText =
          pauta.autorizacion_directiva === true ||
          pauta.autorizacion_directiva === "true"
            ? "Aprobada"
            : "Rechazada";

        const authComercialClass =
          authComercialText === "Aprobada"
            ? "auth-badge--approved"
            : "auth-badge--rejected";

        const authDirectivaClass =
          authDirectivaText === "Aprobada"
            ? "auth-badge--approved"
            : "auth-badge--rejected";
        ...
        row.innerHTML = `
                    ...
                    <td class="monto-cell">$${
                      pauta.monto_de_pauta
                        ? pauta.monto_de_pauta.toLocaleString()
                        : "0"
                    }</td>
                    ...
                    <td class="action-cell">
                        <div class="action-buttons">
                            <button class="btn-action edit" title="Editar pauta" data-pauta-id="${
                              pauta.id
                            }">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-action delete" title="Eliminar pauta" data-pauta-id="${
                              pauta.id
                            }">
        ...
        row
          .querySelector(".edit")
          .setAttribute("data-pauta-id", pauta.id_pauta);
        row
  ...
  function setupModalEvents() {
    ...
    if (elements.pautaModal) {
      const modalContainer =
        elements.pautaModal.querySelector(".modal-container");
      if (modalContainer) {
    ...
    document
      .querySelector(".pauta-close-modal")
    ...
    document
      .querySelector(".pauta-btn-cancelar")
      ?.addEventListener("click", () => {
        elements.editPautaModal.style.display = "none";
        limpiarModalPauta();
      });
  ...
  async function handleFormSubmit(e) {
    ...
    try {
      ...
      await mostrarNotificacion("success", "¡Pauta creada exitosamente!");
      resetForm(form);
      closeModalHandler();
      cargarTablaPautas();
    } catch (error) {
  ...
  // Asegúrate de tener también los siguientes métodos definidos:
  // - getFormData()
  // - validateFormData()
  // - submitPauta()
  // - resetForm()
  ...
  // Y que esté correctamente conectado al formulario:
  ...
  function getFormData() {
    return {
      ...
      autorizacio_comercial: document.getElementById("autorizacionComercial")
        .value,
      autorizacion_directiva: document.getElementById("autorizacionDirectiva")
        .value,
  ...
  async function abrirModalEdicion(pautaId) {
    ...
    try {
      ...
      const pauta = pautasDataGlobal.lista_pautas.find(
        (p) => p.id_pauta == pautaId
      );
  ...
  document
    .querySelector(".pauta-btn-guardar")
    ?.addEventListener("click", async function (event) {
      ...
      try {
        ...
        const nombrePauta = document.getElementById(
          "pauta-editar-nombre"
        ).value;
        const ubicacionPauta = document.getElementById(
          "pauta-editar-ubicacion"
        ).value;
        const montoDePauta = parseFloat(
          document.getElementById("pauta-editar-monto").value
        );
        const autorizacionComercial =
          document.querySelector('input[name="pauta-auth-comercial"]:checked')
            .value === "true";
        const autorizacionDirectiva =
          document.querySelector('input[name="pauta-auth-directiva"]:checked')
            .value === "true";
        const statusPauta = document.getElementById(
          "pauta-editar-estado"
        ).value;
        const modelo = parseInt(
          document.getElementById("pauta-editar-modelo-id").value
        );
        ...
        const respuesta = await fetch(
          `${API_BASE_URL}/pautas/apipautas/${pautaId}/`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        await mostrarNotificacion(
          "success",
          "La pauta se ha actualizado correctamente"
        );
        ...
        await mostrarNotificacion(
          "error",
          error.message || "Ocurrió un error al actualizar la pauta"
        );
      } finally {
  ...
  async function openReasignarModal() {
    try {
      ...
      elements.selectModelo.innerHTML =
  ...
  async function handleReasignarSubmit(e) {
    ...
    if (!modeloId || !pautaId) {
      mostrarNotificacion(
        "error",
        "Por favor seleccione un modelo y una pauta"
      );
      return;
    }
    ...
    try {
      ...
      submitBtn.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> Reasignando...';

      const pautaSeleccionada = listaPautasGlobal.find(
        (p) => p.id_pauta == pautaId
      );
      ...
      const requestBody = {
        ...
        autorizacio_comercial:
          pautaSeleccionada.autorizacio_comercial === true ||
          pautaSeleccionada.autorizacio_comercial === "true",
        autorizacion_directiva:
          pautaSeleccionada.autorizacion_directiva === true ||
          pautaSeleccionada.autorizacion_directiva === "true",
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (deleteBtn) {
            ...
            await mostrarNotificacion(
              "success",
              "¡La pauta ha sido eliminada correctamente!"
            );
            cargarTablaPautas();
          }
        } catch (error) {
          ...
          await mostrarNotificacion(
            "error",
            error.message || "Ocurrió un error al eliminar la pauta"
          );
        }
      }
    });
